...
public class JadenCasingStrings {
    ...
    static  String toJadenCase(String phrase) {
        ...
        String[] splitStrings = phrase.split(" ");
        StringBuilder sb = new StringBuilder();

        for(String splitString: splitStrings){

            String firstLetter = String.valueOf(splitString.charAt(0)).toUpperCase();

            if(splitString.length() > 1){
                String firstWord = firstLetter + splitString.substring(1);
                sb.append(firstWord);
            }else{
